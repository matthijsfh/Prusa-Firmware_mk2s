...
bool activefilament = false; // FRIMMEL
	...
	case S::GetFinda: //response to command P0
		...
		if (mmu_rx_ok() > 0)
		{
			...
			if ((!mmu_finda && CHECK_FSENSOR && fsensor_enabled) ||
                             (!(READ(IR_SENSOR_PIN) == 0)  && CHECK_FSENSOR && fsensor_enabled && activefilament)) {  // FRIMMEL
...
if (success) { activefilament = true; } // FRIMMEL
